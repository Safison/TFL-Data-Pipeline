...
def upload_latest_status_to_s3(api_status, s3_status):
    ...
    latest_s3_from_date = datetime(1900, 1, 1)
    latest_s3_to_date = datetime(1900, 1, 1)
    latest_s3_modified_date = datetime(1900, 1, 1)

    for i in range(len(s3_status)):
        s3_from_date_str = s3_status[i].get("fromDate")
        s3_to_date_str = s3_status[i].get("toDate")
        s3_modified_str = s3_status[i].get("modified")

        if s3_from_date_str:
            s3_from_date_date = datetime.strptime(s3_from_date_str, date_format)
            if s3_from_date_date > latest_s3_from_date:
                latest_s3_from_date = s3_from_date_date
        if s3_to_date_str:
            s3_to_date_date = datetime.strptime(s3_to_date_str, date_format)
            if s3_to_date_date > latest_s3_to_date:
                latest_s3_to_date = s3_to_date_date
        if s3_modified_str:
            s3_modified_date = datetime.strptime(s3_modified_str, modified_format)
            if s3_modified_date > latest_s3_modified_date:
                latest_s3_modified_date = s3_modified_date
    # print(latest_s3_from_date)
    # print(latest_s3_to_date)
    # print(latest_s3_modified_date)

    if latest_from_date and latest_to_date and latest_modified_date:
        if latest_from_date > latest_s3_from_date or latest_to_date > latest_s3_to_date or latest_modified_date > latest_s3_modified_date:
            upload_tfl_status_to_s3(api_status,bucket)
            return api_status
        else:
            return False    
